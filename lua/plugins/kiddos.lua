return {
  ...
  config = function(_, opts)
    ...
    local generate_commit_message_pt_BR = {
      name = "Generate commit message",
      command_name = "GeminiGenerateCommitMessagePtBR",
    ...
    local generate_commit_message = {
      ...
      get_prompt = function()
        ...
        return string.format(
          "Here are the staged changes:\n\n```diff\n%s\n```\n\nWrite a commit message using the commitizen convention. The title must be at most 50 characters, and the body should wrapped at 72 characters. Wrap the whole message in a code block with language gitcommit.",
          diff
        )
      end,
    }
    ...
    table.insert(opts.instruction.prompts, generate_commit_message_pt_BR)
